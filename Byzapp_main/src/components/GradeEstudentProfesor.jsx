...
//
const GradeEstudentProfesor = ({ exam, studentId }) => {
  ...
  console.log(
    "id del eestudiante en el grade del profesor siuu alaverga pene pene ",
    studentId,
    "exam",
    examId
  );
    ...
    const fetchCalificacion = async () => {
      ...
      try {
        const response = await axios.get(
          `https://localhost:7095/api/Estudiante/CalculateStudentGrade/${studentId}/${examId}`,
          {
            headers: {
        ...
        const answersResponse = await axios.get(
          `https://localhost:7095/api/Estudiante/GetStudentExamAnswers/${examId}/${studentId}`,
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`,
...
export default GradeEstudentProfesor;
